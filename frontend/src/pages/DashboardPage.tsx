...
import { CheckCircle, Circle, Clock, Zap, ChevronRight, Timer, Play, TrendingUp } from 'lucide-react';
...
export function DashboardPage() {
    ...
    const totalTimeSpent = allTodos?.items?.reduce((acc, todo) => {
        if (todo.status === 'completed' && todo.time_elapsed_seconds) {
            return acc + todo.time_elapsed_seconds;
        }
        return acc;
    }, 0) ?? 0;

    // Get in-progress tasks with time tracking
    const inProgressTasks = allTodos?.items?.filter((t) => t.status === 'in_progress') ?? [];
    
    // Calculate average completion time
    const completedWithTime = allTodos?.items?.filter(
        (t) => t.status === 'completed' && t.time_elapsed_seconds
    ) ?? [];
    const avgCompletionTime = completedWithTime.length > 0
        ? completedWithTime.reduce((acc, t) => acc + (t.time_elapsed_seconds || 0), 0) / completedWithTime.length
        : 0;
    ...
    return (
        <div className="px-5 pb-8 max-w-2xl mx-auto lg:max-w-none lg:px-8">
            ...
            {/* Time tracking stats */}
            <div className="grid grid-cols-2 gap-3 mb-6 animate-fade-in" style={{ animationDelay: '0.12s' }}>
            ...
            {inProgressTasks.length > 0 && (
                <div className="mb-6 animate-fade-in" style={{ animationDelay: '0.14s' }}>
                    <h2 className="text-base font-black tracking-tight mb-3 flex items-center gap-2">
                        <Play size={16} style={{ color: 'var(--accent)' }} />
                        Active Tasks
                    </h2>
                    <div className="space-y-2">
                        {inProgressTasks.map((todo) => {
                            const elapsed = todo.started_at 
                                ? Math.floor((currentTime - new Date(todo.started_at).getTime()) / 1000)
                                : 0;
                            return (
                                <div key={todo.id} className="card p-4 flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer"
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {todo.started_at && (
                                            <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
                                                Started {formatRelative(todo.started_at)}
                                            </p>
                                        )}
                                    </div>
